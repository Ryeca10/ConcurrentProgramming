...
do_join(State, Ref, ChatName) ->
	 Result = findChatName(maps:keys(State#cl_st.con_ch), ChatName),
     case Result of
    	true -> {err, State};
    	false -> 
    			io:format("Client: chatroom doesn't already exist ~n"),
...
findChatName([ChatName|_] , ChatName) -> true;
findChatName([_|T] , ChatName) -> findChatName(T, ChatName);
findChatName([] , _ChatName) -> false.

%% executes `/leave` protocol from client perspective
do_leave(State, Ref, ChatName) ->
	case findChatName(maps:keys(State#cl_st.con_ch), ChatName) of
		true ->  {err, State};
    	false -> whereis(server)!{self(), Ref, leave, ChatName},
...
do_msg_send(State, Ref, ChatName, Message) ->
    ChatRoomPID = chatroom_pid(ChatName, State),
     io:format("  PID  ~p~n", [ChatRoomPID]),


    ChatRoomPID! {self(), Ref, message, Message},
    receive
    	{ChatRoomPID, Ref, ack_msg} -> ok
    end,

     {{msg_sent, State#cl_st.nick},State}.


chatroom_pid(ChatName,State) ->
   case catch(maps:find(ChatName, State#cl_st.con_ch)) of
        error -> 
        	io:format("  error in chatroom_pid in client.erl  ~n"),
        	error;
